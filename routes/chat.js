...
==========================================================
 ...
 - Cargar mensajes
 - Enviar mensajes
 - Compatibilidad con frontend VOBIXCHAT
==========================================================
*/
...
router.get('/users/search', async (req, res) => {
  ...
  const currentUserId = req.vobixUser.id;
      ...
      WHERE
        ...
        (
          ...
          (
            ...
            AND LOWER(vobix_id) LIKE LOWER($2)
          )
  ...
  try {
    ...
    const result = await database.query(
      ...
      WHERE
        c.owner_user_id = $1
      ORDER BY
        COALESCE(c.alias, u.username) ASC
      `,
      [userId]
    );
    ...
    return res.status(500).json({
      ok: false,
      msg: 'No se pudieron cargar los contactos'
    });
  }
});
...
router.post('/contacts', async (req, res) => {
  ...
  const ownerUserId = req.vobixUser.id;

  const contactUserId = cleanId(
    req.body.userId
  );
  ...
  if (
    ...
    return res.status(400).json({
      ok: false,
      msg: 'No puedes añadirte a ti mismo'
    });
  }
  ...
  try {
    ...
    return res.status(500).json({
      ok: false,
      msg: 'No se pudo añadir el contacto'
    });
  }
});
...
router.post('/blocks/:userId', async (req, res) => {
  ...
  const blockerUserId = req.vobixUser.id;
  ...
  if (
    ...
    return res.status(400).json({
      ok: false,
      msg: 'No puedes bloquearte a ti mismo'
    });
  }
...
router.delete('/blocks/:userId', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      ok: false,
      msg: 'No se pudo desbloquear el usuario'
    });
  }
});
...
async function createPrivateConversation(
  ...
  if (
    ...
    return res.status(400).json({
      ok: false,
      msg: 'No puedes crear un chat contigo mismo'
    });
  }
  ...
  try {
    ...
    if (blockResult.rows.length > 0) {
      ...
      return res.status(403).json({
        ok: false,
        msg: 'No se puede iniciar esta conversación'
      });
    }
    ...
    return res.status(500).json({
      ok: false,
      msg: 'No se pudo crear la conversación'
    });
...
// Ruta utilizada por la interfaz nueva
router.post(
  '/conversations',
  ...
  try {
    ...
    const conversations =
      result.rows.map(row => ({
        ...
        // Compatibilidad sencilla con frontend
    ...
    return res.status(500).json({
      ok: false,
      msg: 'No se pudieron cargar las conversaciones'
    });
  }
});
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!allowed) {
        return res.status(403).json({
          ok: false,
          msg: 'No tienes acceso a esta conversación'
        });
      }
      ...
      const messages =
        result.rows.map(message => ({
          ...
          // nombres alternativos para frontend
          text:
            message.content,
      ...
      return res.status(500).json({
        ok: false,
        msg: 'No se pudieron cargar los mensajes'
      });
    }
  }
);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (blocked.rows.length > 0) {
        return res.status(403).json({
          ok: false,
          msg: 'No se puede enviar el mensaje'
        });
      }
      ...
      const message = {
        ...
        senderId:
          result.rows[0].sender_user_id,

        senderUsername:
          req.vobixUser.username || null
      };
