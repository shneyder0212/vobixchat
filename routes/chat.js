...
async function searchUsersHandler(
  ...
  /*
    Búsqueda rápida VOBIXCHAT:
    - nombre
    - Vobix ID
    - teléfono

    Para teléfono comparamos también una versión formada
    únicamente por dígitos. Así +34 655 766 134,
    34655766134 y 655766134 pueden localizarse aunque
    el formato escrito sea diferente.
  */
  ...
  try {
    const result =
      await database.query(
        ...
        ORDER BY
          CASE
            WHEN LOWER(COALESCE(username, '')) =
                 LOWER($5)
            THEN 0

            WHEN LOWER(COALESCE(vobix_id, '')) =
                 LOWER($5)
            THEN 1
