...
async function joinMeetHD(){
  ...
  if(!me) return alert("Debes registrarte primero - Obligatorio");
  try{
    localStreamMeet = await navigator.mediaDevices.getUserMedia({
      ...
      audio: { echoCancellation:true, noiseSuppression:true, autoGainControl:true, sampleRate:48000, channelCount:2, suppressLocalAudioPlayback:true, echoCancellationType:"system" }
    });
    document.getElementById('videoGrid').innerHTML='';
    addMeetVideo('yo', localStreamMeet, true);
    socket.emit('meet-join');
    showBan("VobixMeet HD entrelazado 🔒 Sin Ruido Sin Eco");
  }catch(e){ alert("Permite cámara y micro: "+e.message); }
}
function addMeetVideo(id,stream,isMe){
  ...
  v.srcObject=stream; v.autoplay=true; v.playsInline=true; v.muted=isMe; v.style.cssText="width:100%;height:100%;object-fit:cover";
  ...
  wrap.appendChild(v); wrap.appendChild(label); g.appendChild(wrap);
}
function leaveMeet(){
  for(let k in peersMeet){ try{ peersMeet[k].destroy(); }catch{} }
  peersMeet={}; document.getElementById('videoGrid').innerHTML='';
  if(localStreamMeet) localStreamMeet.getTracks().forEach(t=>t.stop());
  socket.emit('meet-leave');
}
async function shareScreen(){
  try{
    let sc=await navigator.mediaDevices.getDisplayMedia({ video:{width:1920,height:1080,frameRate:60}, audio:true });
    addMeetVideo('pantalla',sc,true);
    for(let k in peersMeet){
      try{ let sender = peersMeet[k]._pc.getSenders().find(s=>s.track && s.track.kind==='video'); if(sender) sender.replaceTrack(sc.getVideoTracks()[0]); }catch{}
    }
    sc.getVideoTracks()[0].onended=()=>{
      let camTrack=localStreamMeet.getVideoTracks()[0];
      for(let k in peersMeet){ let sender = peersMeet[k]._pc.getSenders().find(s=>s.track && s.track.kind==='video'); if(sender) sender.replaceTrack(camTrack); }
...
socket.on('meet-signal', d=>{
  if(!peersMeet[d.from]){
    ...
    peersMeet[d.from]=p; peersMeet[d.from].signal(d.signal);
  }else{ peersMeet[d.from].signal(d.signal); }
});
