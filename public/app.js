...
function showBan(t){ let b=document.getElementById('ban'); b.innerText=t; b.style.display='block'; setTimeout(()=>b.style.display='none',3500); }
...
async function sendPin(){
  ...
  if(!u||!p) return alert("rellena usuario y telefono");
  let r=await fetch('/send-pin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone:p,username:u})});
  let d=await r.json();
  document.getElementById('msg').innerText=d.ok?"PIN en LOGS de Render":"Error: "+d.msg;
...
function logout(){ localStorage.removeItem('vobix_user'); location.reload(); }
function resizeCanvas(){ let c=document.getElementById('signCanvas'); if(!c) return; c.width=c.offsetWidth; c.height=380; }
function openVault(){ let p=document.getElementById('vaultPin').value; if(p==='1234'){ document.getElementById('vaultContent').style.display='block'; showBan("Vault abierto"); } else showBan("PIN Vault incorrecto - usa 1234"); }
