...
app.get('/api/meet/capacity', requireAuth, (req, res) => {
  res.setHeader('Cache-Control', 'no-store');
  const capacity = getVideoCapacity();
  return res.json({
    ok:true,
    provider:capacity.provider,
    designedParticipants:INTERACTIVE_ROOM_MAX_PARTICIPANTS,
    designedConcurrentConnections:DESIGNED_CONCURRENT_CONNECTIONS,
    configuredConnections:capacity.configuredConnections,
    sfuConfigured:capacity.providerConfigured,
    enterpriseContractConfirmed:capacity.enterpriseContractConfirmed,
    capacityVerified:capacity.capacityVerified,
    operational:capacity.operational,
    note:capacity.operational
      ? 'Capacidad empresarial confirmada y verificada mediante prueba de carga'
      : 'Preparado para 50.000 conexiones simultáneas repartidas entre salas; requiere contrato Enterprise de LiveKit Cloud y prueba de carga aprobada'
  });
});
...
app.get('/api/sfu/status', requireAuth, (req, res) => {
  ...
  res.json({
    ...
    designedConcurrentConnections: capacity.designedConcurrentConnections,
    enterpriseReady: capacity.operational
  });
});
