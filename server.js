...
let users = {};
let pins = {};
let banned = {}; // telefono => hasta cuando baneado
let msgHistory = []; // ultimos 100 mensajes
let userMsgs = {}; // phone => [tiempos]
let spamCount = {}; // phone => contador spam

// Cargar baneos si existe archivo
...
function isSpamming(phone){
  ...
  userMsgs[phone] = userMsgs[phone].filter(t=> now - t < 10000); // ultimos 10 seg
  userMsgs[phone].push(now);

  // 5 mensajes en 10 segundos = spam
  if(userMsgs[phone].length > 5){
    if(!spamCount[phone]) spamCount[phone]=0;
    spamCount[phone]++;
    if(spamCount[phone]>=2){
      banned[phone]= now + 5*60*1000; // baneo 5 min
      saveBanned();
      console.log(`BANEADO ${maskPhone(phone)} por spam 5min`);
...
app.post('/send-pin',(req,res)=>{
  ...
  phone=phone.replace(/\s/g,'');
  if(banned[phone] && banned[phone] > Date.now()) return res.json({ok:false,msg:`Baneado ${Math.ceil((banned[phone]-Date.now())/60000)} min por spam`});
  ...
  s.on('chat', d=>{
    ...
    if(containsSpam(d.text)){
      ...
      if(spamCount[phone]>=3){
        ...
        saveBanned();
        s.emit('banned',{msg:"Baneado 10 min por contenido spam"});
        return;
      }
    ...
    // Guardar y reenviar (cifrado, el server no lee)
...
http.listen(PORT, ()=> console.log("VobixChat BLINDADO E2E + ANTI-SPAM LISTO en "+PORT));