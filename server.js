...
app.post('/send-pin', async (req, res) => {
  try {
    ...
    if (!response.ok) return res.status(500).json({ error: 'Error SMS', details: data });
    res.json({ ok: true });
...
app.post('/verify-pin', (req, res) => {
  ...
  if (Date.now() > saved.expires) { pins.delete(phone); return res.status(400).json({ error: 'PIN expirado' }); }
  if (saved.pin !== pin) return res.status(400).json({ error: 'PIN incorrecto' });

  const exists = usersDB.find(u => u.phone === phone);
  if (!exists) {
    usersDB.push({ phone, date: new Date().toLocaleString(), country: saved.country, lang: lang || saved.lang || 'es' });
  }
  pins.delete(phone);
  res.json({ ok: true });
});
...
app.get('/admin/users', (req, res) => {
  if (req.headers['x-admin-key'] !== ADMIN_KEY) return res.status(403).json({ error: 'Clave admin incorrecta' });
  res.json({ users: usersDB, total: usersDB.length });
});

const PORT = process.env.PORT || 10000;
app.listen(PORT, () => console.log(`VOBIXCHAT corriendo puerto ${PORT}`));