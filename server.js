...
function checkAdmin(req, res, next) {
  if ((req.query.key || req.headers['x-admin-key']) === ADMIN_KEY) return next();
...
app.post('/send-pin', (req, res) => {
  ...
  phone = phone.replace(/\s/g, '');
  if (!isRealMobile(phone)) return res.json({ ok: false, msg: "Solo móviles reales" });
  let pin = Math.floor(100000 + Math.random() * 900000).toString();
  pins[phone] = pin;
  if (!users[phone]) {
    users[phone] = { username, phone, verified: false, gratis: NUMEROS_GRATIS.includes(phone), publicKey: null, created: Date.now() };
  } else {
...
// E2E - GUARDAR LLAVE PUBLICA
app.post('/save-key', (req, res) => {
  let { phone, publicKey } = req.body;
  if (phone && publicKey && users[phone]) {
    users[phone].publicKey = publicKey;
  }
  res.json({ ok: true });
});
...
app.get('/api/keys', (req, res) => {
  let keys = Object.values(users).map(u => ({
    username: u.username,
...
// SOCKET - SOLO REENVIA CIFRADO
io.on('connection', s => {
  s.on('chat', d => {
    if (!d.username ||!d.text) return;
    io.emit('chat', d);
  });
});

const PORT = process.env.PORT || 3000;
http.listen(PORT, () => console.log("VobixChat E2E LISTO en " + PORT));